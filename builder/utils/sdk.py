...
def get_sdk_version(env):
    ...
    try:
        ...
        return "v" + str(semver.Version(requested_version).truncate())
    except:
        package_version = version.get_original_version(
            platform.get_package_version("framework-zephyr").split("+")[0]
        )
        return "v" + str(semver.Version(package_version).truncate())
